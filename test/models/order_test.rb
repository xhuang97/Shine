  ...
  # should_not allow_value(3.14159).for(:date)
   ...
   context "Within context" do
    ...
    should "verify that the date is set for today unless otherwise specified" do
      assert_equal @alex_o1.date, 5.months.ago.to_date  # a specified date is unchanged
      walter_o3 = FactoryGirl.create(:order, user: @walter, school: @ingomar, grand_total: 100.00, date: nil)
      assert_equal walter_o3.date, Date.current  # an order without any date is set to today by default
      walter_o3.delete
      sam_o2 = FactoryGirl.create(:order, user: @sam, school: @cent_cath, grand_total: 100.00, date: "groundhog day")
      assert_equal sam_o2.date, Date.current  # an order without a legitimate date is set to today by default
      sam_o2.delete
    end
